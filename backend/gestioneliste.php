...
/** Verifica che la lista appartenga all'utente (solo owner). */
...
/** Verifica che l'utente sia membro della lista (ma non owner). */
function isMember(PDO $pdo, int $my_id, int $lista_id): bool
{
...
/** Owner O membro — può aggiungere/rimuovere/riordinare film. */
function canEditList(PDO $pdo, int $my_id, int $lista_id): bool
{
    return ownsList($pdo, $my_id, $lista_id) || isMember($pdo, $my_id, $lista_id);
}
...
function ensureFilmExistsList(PDO $pdo, int $tmdb_id): int|false
{
    ...
    // Fallback: inserisce record minimo
    $s = $pdo->prepare("INSERT INTO Film (TMDB_ID, Title) VALUES (?, ?)");
    $s->execute([$tmdb_id, 'Film sconosciuto']);
    return (int)$pdo->lastInsertId();
}

/** Prossima posizione disponibile in una lista. */
...
try {
    switch ($action) {
        ...
        case 'crea_lista':
            ...
            $pdo->prepare("INSERT INTO Lista (Titolo, Descrizione, IDUtente) VALUES (?,?,?)")
                ->execute([mb_substr($titolo,0,255), mb_substr($desc,0,255) ?: null, $my_id]);
            $new_id = (int)$pdo->lastInsertId();
            echo json_encode(['ok' => true, 'lista_id' => $new_id]);
            break;

        // ── Modifica lista (solo owner) ─────────────────────────
        case 'modifica_lista':
        ...
        case 'aggiungi_film':
            ...
            if (!canEditList($pdo, $my_id, $lista_id)) {
